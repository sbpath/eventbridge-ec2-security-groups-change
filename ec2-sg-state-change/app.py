...
def get_username_and_check_group(event):
    ...
    if (
        "userIdentity" in event["detail"]
        and "userName" in event["detail"]["userIdentity"]
    ):
        username = event["detail"]["userIdentity"]["userName"]
    # If userIdentity doesn't have userName, try to get from sessionIssuer
    elif (
        "userIdentity" in event["detail"]
        and "sessionContext" in event["detail"]["userIdentity"]
        and "sessionIssuer" in event["detail"]["userIdentity"]["sessionContext"]
    ):
        username = event["detail"]["userIdentity"]["sessionContext"]["sessionIssuer"][
            "userName"
        ]
    ...
    if "Users" in response and any(
        user["UserName"] == username for user in response["Users"]
    ):
        print(
            f"User '{username}' is part of '{GROUP_NAME_TO_CHECK}' group. Skipping logic."
        )
        return {
            "statusCode": 200,
            "body": json.dumps(
                f"User is part of {GROUP_NAME_TO_CHECK} group. Skipping logic."
            ),
        }
    ...
    if "detail" in event and "eventName" in event["detail"]:
        ...
        if event_name in [
            ...
            if event_name == "ModifySecurityGroupRules":
                security_group_id = event["detail"]["requestParameters"][
                    "ModifySecurityGroupRulesRequest"
                ]["GroupId"]
                # Extract the rule details
                rule = event["detail"]["requestParameters"][
                    "ModifySecurityGroupRulesRequest"
                ]["SecurityGroupRule"]
                ...
                for ip_permission in ip_permissions:
                    ...
                    if (
                        "ipRanges" in ip_permission
                        and "items" in ip_permission["ipRanges"]
                    ):
                    ...
                    if (
                        "ipv6Ranges" in ip_permission
                        and "items" in ip_permission["ipv6Ranges"]
                    ):
            ...
            for rule in rule_details:
                ...
                if rule["CIDR IP"] in [
                    "0.0.0.0/0",
                    "::/0",
                ]:  # Check for both IPv4 and IPv6 '0.0.0.0/0' or '::/0'
                    ...
                    try:
                        ...
                        if event_name == "AuthorizeSecurityGroupIngress":
                            ...
                            ec2_client.authorize_security_group_ingress(
                                GroupId=security_group_id, IpPermissions=[new_rule_ipv4]
                            )
                            print(
                                f"Updated ingress rule in security group {security_group_id} with CIDR: {PREDEFINED_CIDR}"
                            )
                            ...
                            print(
                                f"Updated egress rule in security group {security_group_id} with CIDR: {PREDEFINED_CIDR}"
                            )
                            ...
                            print(
                                f"Updated security group rule in security group {security_group_id} with CIDR: {PREDEFINED_CIDR}"
                            )
